...
hash_t *hash_crear(size_t capacidad_inicial)
{
	...
	else hash->capacidad_inicial = capacidad_inicial;
 ...
 */
bool inserto_en_primera_posicion(hash_t *hash, size_t posicion, char *clave, void *valor, void **encontrado)
{
...
bool hash_insertar(hash_t *hash, char *clave, void *valor, void **encontrado)
{
    if (!hash || !clave) 
        return false;
    if ((double)hash->ocupado / (double)hash->capacidad_inicial > MAX_FACTOR_CARGA) {
    ...
    if (!hash->tabla[posicion]) {
        return inserto_en_primera_posicion(hash, posicion, clave, valor, encontrado);
    }
	...
	while((i < hash->capacidad_inicial) && (sigue_iterando)){
		...
		if(auxiliar){
			while(auxiliar && (sigue_iterando)){
				contador++;
				if( f(auxiliar->clave,auxiliar->elemento,ctx) == false){
